...
var groupEventTypeColumnIndex;
var isFromMeColumnIndex;
var groupMemberColumnIndex;
var messageDateColumnIndex;
var messageTextColumnIndex;
var pushNameColumnIndex; // "From" name in a group message
...
function readSingleFile(e) {
	...
	reader.onload = function(e) {
		...
		partnerTypeColumnIndex = getColumIndexByName("ZWACHATSESSION", "ZPARTNERNAME");
		...
		groupMemberColumnIndex = getColumIndexByName("ZWAMESSAGE", "ZGROUPMEMBER");
		...
		pushNameColumnIndex = getColumIndexByName("ZWAMESSAGE", "ZPUSHNAME");
...
function displayConversationsList() {
	...
	for (i = 0; i < dbResult[0].values.length; i++) {
		...
		html += '<a onclick="displayConversationDetail(' + conversationId + ')" href="#">' + dbResult[0].values[i][partnerTypeColumnIndex] + '</a>';
		html += '</div>';
	}
...
function displayConversationDetail(id) {
	...
	for (i = 0; i < dbResult[0].values.length; i++) {
		...
		if (dbResult[0].values[i][12] == 0 && dbResult[0].values[i][groupMemberColumnIndex]) {
			html += '<div class="from-name-in-group">' + dbResult[0].values[i][pushNameColumnIndex] + '</div>';
		}
