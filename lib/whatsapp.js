...
function displayConversationMessages(id) {
  ...
  var dbResult = db.exec('SELECT ZPARTNERNAME FROM ZWACHATSESSION WHERE Z_PK = ' + id);
  var ZPARTNERNAME = dbResult[0].values[0];

  dbResult = db.exec('SELECT COUNT(*) FROM ZWAMESSAGE WHERE ZCHATSESSION = ' + id);
  var messageCount = dbResult[0].values[0];

  document.getElementById('partner-name').innerHTML = ZPARTNERNAME + ' (' + messageCount + ')';

  dbResult = db.exec(`SELECT
                        ZGROUPEVENTTYPE,
                        replace(ZTEXT, '\n', '<br>'),
                        ZISFROMME,
                        ZGROUPMEMBER,
