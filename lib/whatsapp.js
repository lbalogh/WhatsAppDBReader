...
function displayConversationMessages(id) {
  ...
  var stmt = db.prepare('SELECT COUNT(*) AS COUNT FROM ZWAMESSAGE WHERE ZCHATSESSION = $ZCHATSESSION');
  var row = stmt.getAsObject({$ZCHATSESSION:id});

  document.getElementById('conversation-title').innerHTML = document.getElementById(id).getElementsByClassName('partner-name')[0].innerHTML + ' (' + row['COUNT'] + ')';
  ...
  if (filter) {
    sql += 'AND (ZTEXT LIKE $filter COLLATE NOCASE OR ZPUSHNAME LIKE $pushName COLLATE NOCASE)';
  }

  stmt = db.prepare(sql);
  stmt.bind({$ZCHATSESSION:id, $filter:'%' + filter + '%', $pushName:'%' + filter + '%'});
