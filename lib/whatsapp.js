var db;
var timer;

document.getElementById('file-input').addEventListener('change', readSingleFile, false);

// Read the sqlite file selected in the open dialog
function readSingleFile(e) {
	var file = e.target.files[0];
	if (!file) {
		return;
	}

	var reader = new FileReader();
	reader.onload = function(e) {
		db = new SQL.Database(e.target.result);
		displayConversationsList();
	};
	reader.readAsBinaryString(file);
}

// Display the conversations list in the left pane
function displayConversationsList() {
	var dbResult = db.exec('SELECT DISTINCT ZWACHATSESSION.Z_PK, ZWACHATSESSION.ZPARTNERNAME, ZWACHATSESSION.ZLASTMESSAGEDATE FROM ZWACHATSESSION, ZWAMESSAGE WHERE ZWACHATSESSION.Z_PK = ZWAMESSAGE.ZCHATSESSION AND ZWACHATSESSION.ZHIDDEN = 0 AND ZWACHATSESSION.ZETAG IS NOT NULL ORDER BY ZWACHATSESSION.ZLASTMESSAGEDATE DESC');

	var html = '';
	for (i = 0; i < dbResult[0].values.length; i++) {
		var Z_PK = dbResult[0].values[i][0];
		var ZPARTNERNAME = dbResult[0].values[i][1];
		var ZLASTMESSAGEDATE = dbResult[0].values[i][2];

		var lastMessageDate = new Date((978307200 + ZLASTMESSAGEDATE) * 1000);

		html += '<a onclick="displayConversationDetail(' + Z_PK + ')" href="#">';
		html += '<div class="conversation">';
		html += ZPARTNERNAME;
		html += '<div class="last-message-date">' + formatDate(lastMessageDate) + '</div>'
		html += '</div>';
		html += '</a>';
	}

	document.getElementById('left-pane').innerHTML = html;
}

// Display the selected conversation in the right pane
function displayConversationDetail(id) {
	var dbResult = db.exec('SELECT ZGROUPEVENTTYPE, ZTEXT, ZISFROMME, ZGROUPMEMBER, ZMESSAGEDATE, ZPUSHNAME FROM ZWAMESSAGE WHERE ZCHATSESSION = ' + id);
	//console.log(dbResult);

	var html = '';
	var previousMessageDate;

	for (i = 0; i < dbResult[0].values.length; i++) {
		var ZGROUPEVENTTYPE = dbResult[0].values[i][0];
		var ZTEXT = dbResult[0].values[i][1];

		if (ZGROUPEVENTTYPE != 0 || !ZTEXT) {
			continue;
		}

		var ZISFROMME = dbResult[0].values[i][2];
		var ZGROUPMEMBER = dbResult[0].values[i][3];
		var ZMESSAGEDATE = dbResult[0].values[i][4];
		var ZPUSHNAME = dbResult[0].values[i][5];

		var messageDate = new Date((978307200 + ZMESSAGEDATE) * 1000);
		if (!previousMessageDate || messageDate.getMonth() != previousMessageDate.getMonth() || messageDate.getDate() != previousMessageDate.getDate()) {
			previousMessageDate = messageDate;
			html += '<div class="message timestamp">' + formatDate(messageDate) + '</div>';
		}

		html += '<div class="message ' + (ZISFROMME == 1 ? 'message-from-me' : 'message-from-other') + '">';

		if (ZISFROMME == 0 && ZGROUPMEMBER) {
			html += '<div class="from-name-in-group">' + ZPUSHNAME + '</div>';
		}

 		html += ZTEXT.replace('\n', '<br><br>');
		html += '</div>';
	}

	document.getElementById('right-pane').innerHTML = html;

  var filter = document.getElementById('filter-field').value;
	if (filter.length > 0) {
  	filterPane('right-pane', 'message', filter);
	}
}

function pad(num, size) {
	var s = "000" + num;
	return s.substr(s.length - size);
}

function formatDate(date) {
	return pad(date.getDate(), 2) + '.' + pad(date.getMonth() + 1, 2) + '.' + date.getFullYear();
}

function filterFieldChanged() {
  clearTimeout(timer);
	timer = setTimeout(applyFilter, 750);
	
}

function applyFilter() {
  console.log(applyFilter);
  var filter = document.getElementById('filter-field').value;
  filterPane('right-pane', 'message', filter);
}

function filterPane(pane, divClass, filter) {
	var messages = document.getElementById(pane).getElementsByTagName('div');
	var lastTimestamp;

  filter = filter.toUpperCase();

	for (i = 0; i < messages.length; i++) {
		if (!messages[i].classList.contains(divClass)) {
			continue;
		}

		if (filter.length == 0) {
      messages[i].style.display = '';
		}
		else {
			if (messages[i].classList.contains('timestamp')) {
	      lastTimestamp = messages[i];
			}

	    if (messages[i].innerHTML.toUpperCase().indexOf(filter) > -1) {
					if (lastTimestamp) {
	        	lastTimestamp.style.display = '';
					}
					messages[i].style.display = '';
	    } else {
					messages[i].style.display = 'none';
	    }
		}
	}
}
